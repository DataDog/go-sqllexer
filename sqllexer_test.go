...
here */`,
		...
		{
			// Firebird follows ANSI string semantics: no backslash escapes,
			// a literal quote is escaped by doubling it ('').
			name:  "firebird string with doubled quote",
			input: `SELECT 'don''t' FROM t`,
			expected: []TokenSpec{
				{COMMAND, "SELECT"},
				{SPACE, " "},
				{STRING, `'don''t'`},
			...
			lexerOpts: []lexerOption{WithDBMS(DBMSFirebird)},
		},
		{
			// In Firebird, backslash is an ordinary character, so ESCAPE '\'
			// must terminate at its closing quote even when more SQL follows.
			name:  "firebird escape backslash mid-query",
			input: `SELECT * FROM t WHERE x LIKE '%a%' ESCAPE '\' AND y = 1`,
			expected: []TokenSpec{
				{COMMAND, "SELECT"},
				{SPACE, " "},
		...
		{
			// Firebird drivers use :name bind parameters
			name:  "firebird bind parameters",
			input: `SELECT * FROM t WHERE id = :id AND name = :name`,
			expected: []TokenSpec{
				{COMMAND, "SELECT"},
				{SPACE, " "},
				{WILDCARD, "*"},
		...
		{
			// '$' is part of Firebird identifiers (system tables like RDB$...,
			// user-defined names like MY$VAR)
			name:  "firebird identifiers containing dollar",
			input: `SELECT RDB$RELATION_NAME, MY$VAR FROM t`,
			expected: []TokenSpec{
				{COMMAND, "SELECT"},
				{SPACE, " "},
				{IDENT, "RDB$RELATION_NAME"},
				{PUNCTUATION, ","},
				{SPACE, " "},
				{IDENT, "MY$VAR"},
				{SPACE, " "},
		...
		{
			// Firebird has no dollar-quoted strings and no $n parameters; a
			// leading '$' is not the start of a PostgreSQL-style literal
			name:  "firebird dollar is not a dollar-quoted string",
			input: `SELECT $1, $tag$ FROM t`,
			expected: []TokenSpec{
				{COMMAND, "SELECT"},
				{SPACE, " "},
				{UNKNOWN, "$"},
		...
		{
			// By contrast, without a DBMS the same input still produces
			// PostgreSQL-style tokens
			name:  "dollar quoted string without dbms",
			input: `SELECT $tag$ content $tag$ FROM t`,
			expected: []TokenSpec{
				{COMMAND, "SELECT"},
				{SPACE, " "},
				{DOLLAR_QUOTED_STRING, `$tag$ content $tag$`},
				{SPACE, " "},
				{KEYWORD, "FROM"},
				{SPACE, " "},
				{IDENT, "t"},
		...
		{
			name:  "firebird RECREATE classified as COMMAND",
			input: "RECREATE TABLE test1 (id INTEGER)",
			expected: []TokenSpec{
				{COMMAND, "RECREATE"},
				{SPACE, " "},
				{KEYWORD, "TABLE"},
		...
		{
			name:  "firebird PSQL keywords",
			input: "EXECUTE BLOCK AS BEGIN x = 1; SUSPEND; END",
			expected: []TokenSpec{
				{COMMAND, "EXECUTE"},
				{SPACE, " "},
				{IDENT, "BLOCK"},
		...
		{
			// multi-byte letters in string literals, quoted identifiers and
			// bind parameter names must not split the scan
			name:  "firebird utf-8 string, quoted ident and bind parameter",
			input: `SELECT "ИД" FROM t WHERE note = 'Съешь' AND x > :größe`,
			expected: []TokenSpec{
				{COMMAND, "SELECT"},
				{SPACE, " "},
				{QUOTED_IDENT, `"ИД"`},
				...
				{OPERATOR, ">"},
				{SPACE, " "},
				{BIND_PARAMETER, ":größe"},
			},
			lexerOpts: []lexerOption{WithDBMS(DBMSFirebird)},
		},
		{
			// Backslash-escape dialects (MySQL here) keep treating '' as the
			// end of a literal followed by the start of a new one
			name:  "mysql doubled quote still splits strings",
